...
<div class="page-header"><div><h1 class="page-title">تعديل المطالبة <?= clean($claim['claim_number']) ?></h1></div><div class="page-actions"><a href="<?= url('claims','show',['id'=>$claim['id']]) ?>" class="btn btn-outline"><i class="fas fa-arrow-right"></i> رجوع</a></div></div>
<div class="card"><div class="card-body">
<form method="POST" action="<?= url('claims','update') ?>"><?= csrfField() ?><input type="hidden" name="id" value="<?= $claim['id'] ?>">
    <div class="form-row">
        <div class="form-group"><label class="form-label">العميل</label>
            <select name="client_id" class="form-control" required onchange="loadCompanies(this.value,'company_id')">
                <?php foreach($clients as $c): ?><option value="<?= $c['id'] ?>" <?= $claim['client_id']==$c['id']?'selected':'' ?>><?= clean($c['name']) ?></option><?php endforeach; ?>
            </select></div>
        <div class="form-group"><label class="form-label">الشركة</label>
            <select name="company_id" class="form-control" id="company_id"><option value="">اختر</option>
                <?php foreach($companies as $co): ?><option value="<?= $co['id'] ?>" <?= $claim['company_id']==$co['id']?'selected':'' ?>><?= clean($co['name_ar']) ?></option><?php endforeach; ?>
            </select></div>
    </div>
    <div class="form-row">
        <div class="form-group"><label class="form-label">تاريخ الاستحقاق</label><input type="date" name="due_date" class="form-control" value="<?= $claim['due_date'] ?>"></div>
        <div class="form-group"><label class="form-label">نسبة المطالبة (%)</label><input type="number" name="claim_percentage" class="form-control" value="<?= $claim['claim_percentage'] ?? 100 ?>" min="0" max="100" step="0.01"></div>
    </div>
    <div class="form-group"><label class="form-label">الحالة</label><select name="status" class="form-control">
        <?php foreach(['draft'=>'مسودة','sent'=>'مرسلة','due'=>'مستحقة','overdue'=>'متأخرة','partially_paid'=>'مسددة جزئياً','paid'=>'مسددة','cancelled'=>'ملغية'] as $k=>$v): ?>
    ...
    <tbody id="items-tbody">
        <?php foreach($claim['items'] as $i => $item): ?>
        <tr id="item-row-<?= $i ?>">
            <td>
    ...
    <div class="grid-2 mt-3"><div>
        <div class="form-group"><label class="form-label">ملاحظات</label><textarea name="notes" class="form-control" rows="2"><?= clean($claim['notes']) ?></textarea></div>
    </div><div class="card" style="background:var(--bg-input);"><div class="card-body">
        <div class="quick-stat"><span>الفرعي</span><input type="number" name="subtotal" id="subtotal" class="form-control" value="<?= $claim['subtotal'] ?>" readonly style="width:140px;text-align:left;direction:ltr;"></div>
        <div class="quick-stat"><span>الضريبة (%)</span><input type="number" name="vat_rate" id="vat_rate" class="form-control" value="<?= $claim['vat_rate'] ?>" style="width:80px;" onchange="recalcSubtotal()"></div>
        <div class="quick-stat"><span>قيمة الضريبة</span><input type="number" name="vat_amount" id="vat_amount" class="form-control" value="<?= $claim['vat_amount'] ?>" readonly style="width:140px;text-align:left;direction:ltr;"></div>

        <div class="quick-stat" style="border-top:2px solid var(--gold);padding-top:12px;"><span class="text-bold text-gold">الإجمالي</span><input type="number" name="total" id="total" class="form-control text-bold" value="<?= $claim['total'] ?>" readonly style="width:140px;text-align:left;direction:ltr;color:var(--gold);"></div>
    </div></div></div>
...
<script>
let claimItemIndex = <?= count($claim['items']) ?>;
